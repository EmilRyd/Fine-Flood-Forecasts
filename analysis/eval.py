...
import numpy as np
from utils import TrainedModel, TrainedModelID, NUM_BASINS
...
def processed_model_csv(eval_file, basins: list):
    # read csv
    ...
    # assert proper dataset length
    # TODO figure out a way to resurrect this assert
    #assert set(basins) == set(df.basin) or (len(basins) == 0 and len(df.basin) == NUM_BASINS[dataset]), f'All basins are not in dataframe, or vice versa. df_length = {len(df.basin)}, basins_length = {len(basins)}'
    ...
    # Drop rows with NaN values (originally inf values)
...
def evalute_model_csvs(model_eval_files: dict, basins: list, include_benchmark: bool, bolden_values: bool):
    ...
    for model, (eval_file, _) in model_eval_files.items():

        assert os.path.exists(eval_file), "Metric eval file does not exist. Did you run the eval script yet?"
        df = processed_model_csv(eval_file=eval_file, basins=basins)
...
def get_model_eval_files(models: list, basins: list, ignore_previous_metrics: bool, period: str, new_basin_file: Path = None):

    """Takes list of TrainedModel objects and evaluates them against each other, returning a summary DataFrame"""
    models_dict = {}
    for model in models:
        
        assert isinstance(model, TrainedModel), 'model is not a TrainedModel data object'

        # if any models are not evaluated yet, do so
        ...
        if os.path.exists(metrics_file) and not ignore_previous_metrics:
            ...
            if not (set(basins).issubset(set(df.basin.tolist())))  or (not (len(df.basin) == NUM_BASINS[model.cfg.dataset]) and len(basins)==0):  
                if torch.cuda.is_available():
                    eval_run(model.run_dir, period=period, epoch=model.epoch, new_basin_file=new_basin_file)
                else:
                    eval_run(model.run_dir, period=period, epoch=model.epoch, gpu=-1, new_basin_file=new_basin_file)
        else:
            if torch.cuda.is_available():
                eval_run(model.run_dir, period=period, epoch=model.epoch, new_basin_file=new_basin_file)
            else:
        ...
        models_dict[model.config_id] = (metrics_file, model.cfg.dataset)
    return models_dict

def evaluate_models_summary(models: list, basins: list = [], include_benchmark: bool = True, period='test', bolden_values=False, ignore_previous_metrics=False, new_basin_file=None):
        
    models_dict = get_model_eval_files(models=models, basins=basins, ignore_previous_metrics=ignore_previous_metrics, period=period, new_basin_file=new_basin_file)
    # evaluate the model csvs
    df = evalute_model_csvs(models_dict, basins=basins, include_benchmark=include_benchmark, bolden_values=bolden_values)

    return df

def evaluate_model_full(model: TrainedModel, basins: list = [], period='test', ignore_previous_metrics=False, new_basin_file=None):
    models_dict = get_model_eval_files(models=[model], basins=basins, ignore_previous_metrics=ignore_previous_metrics, period=period, new_basin_file=new_basin_file)
    full_df = processed_model_csv(eval_file=models_dict[model.config_id][0],  basins=basins)
